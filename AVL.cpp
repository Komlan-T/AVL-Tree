...
struct AVL {
	...
	void searchName_Helper(Node* root, string name);
...
void AVL::searchName_Helper(Node* root, string name){
	...
	if (nameExist(_root, name)) {
		if (root == nullptr) {
			...
			searchName_Helper(root->left, name);
			searchName_Helper(root->right, name);
		}
	}
	else {
		cout << "unsuccessful" << endl;
